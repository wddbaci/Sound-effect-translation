...
class FileProcessor:
    ...
    def generate_new_name(original_filename, translation, mode):
        # 清理翻译结果中的非法字符
        safe_translation = FileProcessor.sanitize_filename(translation)
        base, ext = os.path.splitext(original_filename)
        if mode == "中文_英文":
            new_name = f"{safe_translation}_{original_filename}"
        elif mode == "只中文":
            new_name = f"{safe_translation}{ext}"
        elif mode == "英文_中文":
            new_name = f"{base}_{safe_translation}{ext}"
        else:
            new_name = f"{safe_translation}_{original_filename}"
        return new_name
...
"""
        ...
        try:
            ...
            for root_dir, dirs, files in os.walk(folder):
                for file in files:
                    if self.stop_event.is_set():
                        break
                    # 使用 is_valid_audio_file 过滤 macOS 隐藏文件
                    if FileProcessor.is_valid_audio_file(file):
    ...
    def rename_worker(self, folder, mode, api_key):
        try:
            ...
            for i, (rel_path, chinese) in enumerate(zip(pending_files, final_translations)):
                ...
                # 再次检查文件名，跳过 macOS 隐藏文件（以防万一）
                if os.path.basename(rel_path).startswith('._'):
                    self.log(f"跳过 macOS 系统文件: {rel_path}")
                    continue
    ...
    def translate_folders_worker(self, root_folder, api_key):
        try:
            ...
            self.log(f"共找到 {len(folders)} 个文件夹需要处理")
            ...
            for idx, (rel, orig_name, abs_path) in enumerate(folders):
                ...
                # 清理文件夹名称中的非法字符
                safe_translated = FileProcessor.sanitize_filename(translated)

                parent_dir = os.path.dirname(abs_path)
                new_abs = os.path.join(parent_dir, safe_translated)
